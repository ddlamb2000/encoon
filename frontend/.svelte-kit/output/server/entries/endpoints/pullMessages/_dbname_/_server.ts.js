...
import { k as kafka } from "../../../../chunks/kafka.js";
function newUuid() {
  return crypto.randomUUID();
}
const GET = async ({ params, request, url, cookies }) => {
  const topic = private_env.TOPIC_PREFIX + "-" + params.dbname + "-responses";
  const ac = new AbortController();
  const groupId = private_env.KAFKA_GROUP_ID + "-" + newUuid();

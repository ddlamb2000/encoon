...
func TestIsDbAuthorized(t *testing.T) {
	...
	dbName := "test"
	uuid, firstName, lastName, err := isDbAuthorized(dbName, "root", "dGVzdA==")
	if err != nil {
	...
	uuid, firstName, lastName, err2 := isDbAuthorized(dbName, "rot", "dGVzdA==")
	if err2 == nil {
		t.Errorf("Can authenticate with a wrong id: %v, %v, %v.", uuid, firstName, lastName)
	}
	uuid, firstName, lastName, err3 := isDbAuthorized(dbName, "root", "========")
	if err3 == nil {
		t.Errorf("Can authenticate with a wrong password: %v, %v, %v.", uuid, firstName, lastName)
	}
	uuid, firstName, lastName, err4 := isDbAuthorized("====", "root", "========")
	if err4 == nil {

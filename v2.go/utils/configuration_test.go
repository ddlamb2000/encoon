...
func TestLoadConfiguration(t *testing.T) {
	...
	if !IsDatabaseEnabled(dbName) {
		t.Fatalf("Database %q isn't enabled.", dbName)
	}
	...
	if !reflect.DeepEqual(secret, expected) {
		t.Fatalf("Jwt secret is wrong: %v instead of %v.", secret, expected)
	}
	...
	if root != "root" || password != expectedPassword {
		t.Fatalf("Root or password isn't correct for database %q: %q and %q.", dbName, root, password)
	}

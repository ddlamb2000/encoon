...
do
    ...
    echo -n "$MSG" && go test -v $PACK/* -outputdir testData -covermode=count -cover -coverprofile cover_"$PACK"_test.out && go tool cover -html=testData/cover_"$PACK"_test.out  -o testData/cover_"$PACK"_test.html
done
